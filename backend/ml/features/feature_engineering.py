...
def _row_key(txn_id: Any, created_at: Any) -> tuple[str, str]:
    """Stable physical-row key. txn_id alone is not unique in legacy databases."""
    return (str(txn_id), pd.Timestamp(created_at).isoformat())


def _keys_for_frame(frame: pd.DataFrame) -> list[tuple[str, str]]:
    return [_row_key(tid, ts) for tid, ts in zip(frame[TXN_ID_COLUMN], frame[TIMESTAMP_COLUMN])]
...
def _velocity_features(
    ...
    buyer_sorted["_row_key"] = [_row_key(tid, ts) for tid, ts in zip(buyer_sorted["txn_id"], buyer_sorted["created_at"])]
    buyer_view = buyer_sorted.set_index("_row_key")
    txn_keys = _keys_for_frame(txn_df)
    for col in ["txn_count_buyer_1h", "txn_count_buyer_24h", "txn_count_buyer_7d",
                "amount_sum_buyer_24h", "seconds_since_last_buyer_txn"]:
        out[col] = pd.Series(txn_keys).map(buyer_view[col]).fillna(0).to_numpy(dtype=np.float64)
    ...
    dev_sorted["_row_key"] = [_row_key(tid, ts) for tid, ts in zip(dev_sorted["txn_id"], dev_sorted["created_at"])]
    dev_view = dev_sorted.set_index("_row_key")
    txn_keys = _keys_for_frame(txn_df)
    for col in ["txn_count_device_1h", "txn_count_device_24h", "txn_count_device_7d",
                "distinct_buyers_device_7d", "device_age_days"]:
        out[col] = pd.Series(txn_keys).map(dev_view[col]).fillna(0).to_numpy(dtype=np.float64)
    ...
    ip_sorted["_row_key"] = [_row_key(tid, ts) for tid, ts in zip(ip_sorted["txn_id"], ip_sorted["created_at"])]
    ip_view = ip_sorted.set_index("_row_key")
    txn_keys = _keys_for_frame(txn_df)
    for col in ["txn_count_ip_1h", "txn_count_ip_24h", "distinct_buyers_ip_24h"]:
        out[col] = pd.Series(txn_keys).map(ip_view[col]).fillna(0).to_numpy(dtype=np.float64)
    ...
    bin_sorted["_row_key"] = [_row_key(tid, ts) for tid, ts in zip(bin_sorted["txn_id"], bin_sorted["created_at"])]
    bin_view = bin_sorted.set_index("_row_key")
    txn_keys = _keys_for_frame(txn_df)
    for col in ["txn_count_card_bin_1h", "txn_count_card_bin_24h",
                "distinct_buyers_card_bin_24h"]:
        out[col] = pd.Series(txn_keys).map(bin_view[col]).fillna(0).to_numpy(dtype=np.float64)
...
def _novelty_features(
    ...
    # Compute causally using the physical transaction row (txn_id + created_at).
    # Legacy development data can contain duplicate txn_id values.
    #
    ...
    q = text("""
        WITH ordered AS (
            SELECT
                txn_id, buyer_id, device_id, ip_address, card_bin, created_at,
                ROW_NUMBER() OVER (PARTITION BY buyer_id ORDER BY created_at, txn_id) AS rn
            FROM transactions
        )
        SELECT
            o.txn_id, o.created_at,
    ...
    txn_to_idx = {_row_key(tid, ts): i for i, (tid, ts) in enumerate(zip(txn_df[TXN_ID_COLUMN], txn_df[TIMESTAMP_COLUMN]))}
    for tid, created_at, known_dev, new_ip, new_bin in rows:
        idx = txn_to_idx.get(_row_key(tid, created_at))
        if idx is None:
            continue
...
def _ring_features(db: Session, txn_df: pd.DataFrame) -> dict[str, np.ndarray]:
    ...
    # Pre-fetch the device, ip, bin, buyer for each physical txn row.
    rows = db.execute(text("""
        SELECT txn_id, created_at, buyer_id, device_id, ip_address, card_bin
        FROM transactions
        WHERE txn_id = ANY(:tids)
    """), {"tids": list({k[0] for k in txn_to_idx})}).fetchall()

    for tid, created_at, buyer, dev, ip, bin_ in rows:
        idx = txn_to_idx.get(_row_key(tid, created_at))
        if idx is None:
            continue
    ...
    rows = db.execute(text("""
        ...
        FROM (
            SELECT DISTINCT ON (txn_id)
                txn_id, created_at, amount, method, card_bin,
                ip_address, city, buyer_id, device_id, currency
            FROM transactions
            ORDER BY txn_id, created_at DESC
        ) AS t
