...
def score_transaction(db: Optional[Session], payload: Dict[str, Any]) -> RiskDecisionResponse:
    ...
    allow_t = float(thresholds.get("allow", 0.45))
    challenge_t = float(thresholds.get("challenge", 0.75))
    if db is not None:
        try:
            ...
            if row:
                allow_t, challenge_t = float(row["allow_threshold"]), float(row["challenge_threshold"])
        except Exception:
            logger.warning("Using model thresholds because persisted thresholds could not be read")
    decision_engine = DecisionEngine(allow_t, challenge_t)
