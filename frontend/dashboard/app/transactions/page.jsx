...
export default async function TransactionsPage({ searchParams }) {
  ...
  const all = await api.listTransactions({limit:200})
  const rows = all.filter((t) => {
    if (tier !== 'all' && t.decision !== tier) return false
    if (merchant && !`${t.userName} ${t.userId}`.toLowerCase().includes(merchant.toLowerCase())) return false
    if (date && !String(t.scoredAt).startsWith(date)) return false
    if (range) { const parts=range.split('-').map(Number); if (Number.isFinite(parts[0]) && t.riskScore < parts[0]) return false; if (Number.isFinite(parts[1]) && t.riskScore > parts[1]) return false }
    if (q) {
