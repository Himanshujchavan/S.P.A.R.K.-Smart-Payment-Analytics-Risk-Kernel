...
function ShapBarChart({ features }) {
  ...
  const maxAbs = Math.max(...features.map((f) => Math.abs(Number(f.shap_value))), 0.01)
  return (
      ...
      {features.map((f, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1, background: 'var(--border-hairline)', height: 8, borderRadius: 4, overflow: 'hidden' }}>
            <div
              style={{
                width: `${(Math.abs(Number(f.shap_value)) / maxAbs) * 100}%`,
                height: '100%',
                background: Number(f.shap_value) >= 0 ? 'var(--risk-block)' : 'var(--risk-allow)',
                transition: 'width var(--counter-duration) var(--page-trans-ease)',
              }}
            />
          </div>
          <code style={{ fontFamily: 'var(--font-mono)', width: 40, textAlign: 'right' }}>
            {Number(f.shap_value).toFixed(2)}
          </code>
        </div>
      ))}
    </div>
